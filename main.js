...
let substrateMemory = createField(0.0);
let entities = [];
let tick = 0;
let epoch = 0;
...
const CLIMATES = [
  { name: "Verdant Solstice", icon: "☀️", sunFactor: 1.05, moistureBonus: 0.005, desc: "Optimal sunlight and rapid root mineral synthesis.", rootFreq: 130.81, scale: [261.63, 293.66, 329.63, 392.00, 440.00, 523.25] },
  { name: "Nutrient Monsoon", icon: "🌧️", sunFactor: 0.85, moistureBonus: 0.015, desc: "High rainfall accelerating detritus breakdown into fertile loam.", rootFreq: 110.00, scale: [220.00, 246.94, 277.18, 329.63, 369.99, 440.00] },
  { name: "Arid Eclipse", icon: "🌘", sunFactor: 0.65, moistureBonus: 0.001, desc: "Dimmed canopy light; organisms rely on stored metabolism.", rootFreq: 98.00, scale: [196.00, 233.08, 261.63, 293.66, 349.23, 392.00] },
  { name: "Bioluminescent Bloom", icon: "✨", sunFactor: 1.25, moistureBonus: 0.008, desc: "High energetic excitation stimulating spore proliferation.", rootFreq: 146.83, scale: [293.66, 329.63, 369.99, 440.00, 493.88, 587.33] }
];
...
function initAudioEngine() {
  ...
  try {
    ...
    droneOsc2.frequency.setValueAtTime(currentClimate.rootFreq * 1.503, audioCtx.currentTime);
...
function defaultPlantGenome() {
  return { growthRate: 1.0, crowdingTolerance: 5, nutrientUptake: 0.8, reproThreshold: 7.0, reproCost: 3.5, maintenanceCost: 0.28, mutationRate: 0.08 };
}

function defaultHerbivoreGenome() {
  return { baseMetabolism: 0.40, movementCost: 0.25, sensoryRadius: 4, maxSatiation: 24.0, reproThreshold: 14.0, reproCost: 7.0, biteEfficiency: 5.0, maxAge: 260, mutationRate: 0.08 };
}

function defaultCarnivoreGenome() {
  return { baseMetabolism: 0.50, movementCost: 0.30, huntRadius: 5, maxSatiation: 32.0, reproThreshold: 20.0, reproCost: 11.0, huntEfficiency: 15.0, maxAge: 300, mutationRate: 0.08 };
}

function defaultBenthicGenome() {
  return { baseMetabolism: 0.32, movementCost: 0.18, sensoryRadius: 4, scavengeEfficiency: 3.5, maxSatiation: 20.0, reproThreshold: 13.0, reproCost: 6.5, maxAge: 280, mutationRate: 0.08 };
}
...
function createPlant(x, y, genome = null) {
  return { type: "plant", x, y, energy: 5.0, age: 0, genome: genome ? mutateGenome(genome) : defaultPlantGenome(), pulse: 0, defenseTimer: 0, alive: true };
}
...
function spawnSynapticSpark(fromX, fromY, toX, toY, color = "#00f0ff") {
  ...
  synapticSparks.push({ x: px1, y: py1, targetX: px2, targetY: py2, progress: 0, speed: 0.18 + Math.random() * 0.12, color, radius: 2.2 + Math.random() * 1.5 });
...
function createHerbivore(x, y, genome = null) {
  return { type: "herbivore", x, y, energy: 14.0, age: 0, genome: genome ? mutateGenome(genome) : defaultHerbivoreGenome(), alive: true };
}

function createCarnivore(x, y, genome = null) {
  return { type: "carnivore", x, y, energy: 18.0, age: 0, genome: genome ? mutateGenome(genome) : defaultCarnivoreGenome(), alive: true };
}

function createBenthic(x, y, genome = null) {
  return { type: "benthic", x, y, energy: 12.0, age: 0, genome: genome ? mutateGenome(genome) : defaultBenthicGenome(), alive: true };
}
...
function updatePlant(p) {
  ...
  const memBonus = substrateMemory[p.x][p.y] * 0.08;
...
function logEpoch() {
  ...
  logLine(`📜 Epoch ${epoch} Summary — Flora: ${pCount} | Grazers: ${hCount} | Apex: ${cCount} | Benthic: ${bCount}`, "epoch");
...
function handleCanvasPointer(clientX, clientY) {
  ...
  activeRipples.push({ x: canvasX, y: canvasY, radius: 4, maxRadius: 36, alpha: 0.95, tool: activeTool });
  if (activeTool === "plant") {
...
function renderSynapticFilaments() {
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      ...
      const cardinalTargets = [[(x + 1) % WIDTH, y], [x, (y + 1) % HEIGHT]];
