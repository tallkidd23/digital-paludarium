// =====================================================================
// SYNAPSE REEF v0.5 — Deep Ecology Engine & Neural Plasticity
// Pheromones, Speciation, Benthic Detritivore Guild, & Hebbian Memory
// =====================================================================
...
const INITIAL_DETRITIVORE_COUNT = 8;

// ---------- State ----------
let grid = createGrid();
let soilNutrients = createSoil();
let detritusField = createSoil();
let alarmPheromones = createZeroField(); // Diffusing stress chemical field
let axonWeights = createWeightGrid();    // Hebbian synaptic plasticity matrix

let entities = [];
let tick = 0;
let epoch = 0;

// High-speed visual action potential particles
let synapticSparks = [];
...
let phaseOrbitHistory = []; // { p, h, c, d }
let lastPCount = 0;
let lastHCount = 0;

// Interactive stewardship tool state ('plant' | 'grazer' | 'predator' | 'detritivore' | 'nutrient')
...
function createZeroField() {
  return Array.from({ length: WIDTH }, () => Array(HEIGHT).fill(0.0));
}

function createWeightGrid() {
  // Axon weights connecting to right and down neighbors
  return Array.from({ length: WIDTH }, () =>
    Array.from({ length: HEIGHT }, () => ({ right: 1.0, down: 1.0 }))
  );
}
...
function defaultHerbivoreGenome(morphotype = "standard") {
  const isSprinter = morphotype === "sprinter";
  const isArmored = morphotype === "armored";

  return {
    morphotype: morphotype || "standard",
    baseMetabolism: isSprinter ? 0.35 : (isArmored ? 0.48 : 0.40),
    movementCost: isSprinter ? 0.18 : (isArmored ? 0.35 : 0.25),
    sensoryRadius: isSprinter ? 5 : 4,
    maxSatiation: isArmored ? 30.0 : 24.0,
    reproThreshold: isArmored ? 18.0 : 14.0,
    reproCost: isArmored ? 9.0 : 7.0,
    biteEfficiency: isSprinter ? 6.0 : 5.0,
    armor: isArmored ? 0.65 : 0.0,
    maxAge: 260,
    mutationRate: 0.08,
  };
}

function defaultCarnivoreGenome(morphotype = "stalker") {
  const isAmbush = morphotype === "ambush";

  return {
    morphotype: morphotype || "stalker",
    baseMetabolism: isAmbush ? 0.38 : 0.50,
    movementCost: isAmbush ? 0.45 : 0.30,
    huntRadius: isAmbush ? 3 : 5,
    maxSatiation: 32.0,
    reproThreshold: 20.0,
    reproCost: 11.0,
    huntEfficiency: isAmbush ? 20.0 : 15.0,
    stealth: isAmbush ? 0.75 : 0.0,
    maxAge: 300,
    mutationRate: 0.08,
  };
}

function defaultDetritivoreGenome() {
  return {
    baseMetabolism: 0.25,
    movementCost: 0.15,
    scavengeRadius: 4,
...
function mutateGenome(genome) {
  ...
  for (const key of Object.keys(g)) {
    if (key === "mutationRate" || key === "morphotype" || key === "armor" || key === "stealth") continue;
    ...
    } else if (key === "growthRate" || key === "nutrientUptake" || key === "biteEfficiency" || key === "huntEfficiency" || key === "recycleRate") {
    ...
    } else if (key === "crowdingTolerance" || key === "sensoryRadius" || key === "huntRadius" || key === "scavengeRadius") {
...
function createHerbivore(x, y, genome = null) {
  ...
  if (!genome) {
    const r = Math.random();
    if (r < 0.35) morph = "sprinter";
    else if (r < 0.65) morph = "armored";
  }
  return {
    type: "herbivore",
    x,
    ...
    energy: 14.0,
    age: 0,
    genome: genome ? mutateGenome(genome) : defaultHerbivoreGenome(morph),
    alive: true,
  };
}
...
function createCarnivore(x, y, genome = null) {
  let morph = "stalker";
  if (!genome && Math.random() < 0.4) morph = "ambush";
  return {
    type: "carnivore",
    x,
    y,
    energy: 18.0,
    age: 0,
    genome: genome ? mutateGenome(genome) : defaultCarnivoreGenome(morph),
    alive: true,
  };
}

function createDetritivore(x, y, genome = null) {
  return {
    type: "detritivore",
    x,
    y,
    energy: 10.0,
    age: 0,
    genome: genome ? mutateGenome(genome) : defaultDetritivoreGenome(),
    alive: true,
  };
}
      ...
      if (d > 1 && s > 0.15) {
        for (const [nx, ny] of getNeighbors(x, y)) {
          if (!visited.has(`${nx},${ny}`) && grid[nx]?.[ny]?.type === "plant") {
            if (nx === (x + 1) % WIDTH && ny === y) {
              axonWeights[x][y].right = Math.min(3.0, axonWeights[x][y].right + 0.08);
            } else if (ny === (y + 1) % HEIGHT && nx === x) {
              axonWeights[x][y].down = Math.min(3.0, axonWeights[x][y].down + 0.08);
            }

            const conductivity = (nx === (x + 1) % WIDTH && ny === y) ? axonWeights[x][y].right : 1.0;
            const nextStrength = s * 0.82 * Math.min(1.2, conductivity);

            queue.push({ x: nx, y: ny, s: nextStrength, d: d - 1 });
            if (Math.random() < 0.85) {
...
function decayPlantPulses() {
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      if (axonWeights[x][y].right > 1.0) axonWeights[x][y].right -= 0.001;
      if (axonWeights[x][y].down > 1.0) axonWeights[x][y].down -= 0.001;
    }
  }
}

function updatePheromones() {
  const nextField = createZeroField();
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      const val = alarmPheromones[x][y];
      if (val > 0.01) {
        const spread = val * 0.15;
        nextField[x][y] += val * 0.4;
        for (const [nx, ny] of getNeighbors(x, y)) {
          nextField[nx][ny] += spread / 8;
        }
      }
    }
  }
  alarmPheromones = nextField;
}
...
function initWorld() {
  ...
  alarmPheromones = createZeroField();
  axonWeights = createWeightGrid();
  entities = [];
  timeSeriesHistory = [];
  phaseOrbitHistory = [];
  epochHistory = [];
  ...
  let placedD = 0;
  while (placedD < INITIAL_DETRITIVORE_COUNT) {
    const x = Math.floor(Math.random() * WIDTH);
    const y = Math.floor(Math.random() * HEIGHT);
    if (grid[x][y]) continue;
    const d = createDetritivore(x, y);
    grid[x][y] = d;
    entities.push(d);
    placedD++;
  }
  ...
  clearLog();
  logLine("🌱 Epoch 0 — Synapse Reef v0.5 active: 4-Tier Web, Alarm Pheromones, Speciation & Hebbian Memory.", "epoch");
  updateEpochBadge();
  updateClimateHUD();
}
...
function updateHerbivore(h) {
  ...
  if (!ate) {
    let fleeVector = null;
    let maxAlarm = 0;
    for (const [nx, ny] of getNeighbors(h.x, h.y)) {
      if (alarmPheromones[nx][ny] > maxAlarm) {
        maxAlarm = alarmPheromones[nx][ny];
        fleeVector = { dx: h.x - nx, dy: h.y - ny };
      }
    }

    let bestTarget = null;
    if (maxAlarm > 0.4 && fleeVector) {
      bestTarget = fleeVector;
    } else {
...
function updateCarnivore(c) {
  ...
  for (const [nx, ny] of immediateNeighbors) {
    ...
    if (prey && prey.type === "herbivore" && prey.alive) {
      if (prey.genome.armor && Math.random() < prey.genome.armor) {
        logLine("🛡️ Armored Shell: Grazer deflected apex strike and bolted!", "event");
        alarmPheromones[nx][ny] = Math.min(5.0, alarmPheromones[nx][ny] + 2.0);
        break;
      }
      ...
      alarmPheromones[nx][ny] = Math.min(8.0, alarmPheromones[nx][ny] + 4.0);
...
function updateDetritivore(d) {
  const g = d.genome;
  const immediateNeighbors = [
    [(d.x + 1) % WIDTH, d.y],
    [(d.x - 1 + WIDTH) % WIDTH, d.y],
    [d.x, (d.y + 1) % HEIGHT],
    [d.x, (d.y - 1 + HEIGHT) % HEIGHT],
  ];

  if (detritusField[d.x][d.y] > 0.5) {
    const scavenged = Math.min(detritusField[d.x][d.y], g.recycleRate);
    detritusField[d.x][d.y] -= scavenged;
    soilNutrients[d.x][d.y] = Math.min(10.0, soilNutrients[d.x][d.y] + scavenged * 1.5);
    d.energy = Math.min(g.maxSatiation, d.energy + scavenged * 1.2);
  } else {
    let bestTarget = null;
    let maxDetritus = 0.5;
    const r = Math.floor(g.scavengeRadius);

    for (let dx = -r; dx <= r; dx++) {
      for (let dy = -r; dy <= r; dy++) {
        const sx = (d.x + dx + WIDTH) % WIDTH;
        const sy = (d.y + dy + HEIGHT) % HEIGHT;
        if (detritusField[sx][sy] > maxDetritus && !grid[sx][sy]) {
          maxDetritus = detritusField[sx][sy];
          bestTarget = { dx, dy };
        }
      }
    }
    ...
    if (bestTarget) {
      const stepX = bestTarget.dx !== 0 ? (bestTarget.dx > 0 ? 1 : -1) : 0;
      const stepY = bestTarget.dy !== 0 ? (bestTarget.dy > 0 ? 1 : -1) : 0;
      const candX = (d.x + stepX + WIDTH) % WIDTH;
      const candY = (d.y + stepY + HEIGHT) % HEIGHT;
      if (!grid[candX][candY]) movePos = [candX, candY];
    }

    if (!movePos) {
      const openSpots = immediateNeighbors.filter(([nx, ny]) => !grid[nx][ny]);
      if (openSpots.length > 0) movePos = openSpots[Math.floor(Math.random() * openSpots.length)];
    }
    ...
    if (movePos) {
      ...
      d.y = movePos[1];
      grid[d.x][d.y] = d;
      d.energy -= g.movementCost;
    }
  }

  d.energy -= g.baseMetabolism;
  d.age++;

  if (d.energy >= g.reproThreshold) {
    const pos = randomEmptyNeighbor(d.x, d.y);
    if (pos) {
      const [nx, ny] = pos;
      const child = createDetritivore(nx, ny, g);
      grid[nx][ny] = child;
      entities.push(child);
      d.energy -= g.reproCost;
    }
  }

  if (d.energy <= 0 || d.age > g.maxAge) {
    d.alive = false;
    soilNutrients[d.x][d.y] = Math.min(10.0, soilNutrients[d.x][d.y] + 2.0);
  }
}
...
function environmentalBalance() {
  ...
  const detCount = entities.filter(e => e.type === "detritivore").length;
  ...
  if (detCount < 3 && Math.random() < 0.06) {
    const rx = Math.floor(Math.random() * WIDTH);
    const ry = Math.floor(Math.random() * HEIGHT);
    if (!grid[rx][ry] && detritusField[rx][ry] > 2.0) {
      const d = createDetritivore(rx, ry);
      grid[rx][ry] = d;
      entities.push(d);
      logLine("🦀 Abyssal Awakening: Benthic detritivore emerged to recycle organic fall.", "event");
    }
  }
}
...
function updateMathTelemetry(pCount, hCount, cCount, dCount) {
  ...
  phaseOrbitHistory.push({ p: pCount, h: hCount, c: cCount, d: dCount });
...
function recordTimeSeries() {
  ...
  const dCount = entities.filter(e => e.type === "detritivore").length;

  timeSeriesHistory.push({ p: pCount, h: hCount, c: cCount, d: dCount });
  if (timeSeriesHistory.length > MAX_GRAPH_POINTS) {
    timeSeriesHistory.shift();
  }

  updateMathTelemetry(pCount, hCount, cCount, dCount);
}
...
function step() {
  ...
  for (const e of entities) {
    ...
    else if (e.type === "detritivore") updateDetritivore(e);
  }
...
function logEpoch() {
  ...
  const dets = entities.filter(e => e.type === "detritivore");

  const pCount = plants.length;
  const hCount = herbs.length;
  const cCount = carns.length;
  const dCount = dets.length;

  epochHistory.push({ epoch, pCount, hCount, cCount, dCount });
  if (epochHistory.length > 20) epochHistory.shift();

  logLine(`Epoch ${epoch} (Tick ${tick}) — Canopy: ${pCount} | Grazers: ${hCount} | Apex: ${cCount} | Benthic: ${dCount}`, "epoch");
  ...
  if (epochHistory.length >= 6) {
    ...
    if (cCount > 2 && hTrend < -15) {
      logLine("⚡ Apex Pressure: Predators regulating grazer population; alarm pheromones active.", "event");
    } else if (pTrend > 25 && hTrend > 8 && cCount > 0) {
      logLine("⚖️ Four-Tier Harmony: Balanced trophic web across canopy, herd, pack, and abyss.", "event");
    }
  }
}
...
function handleCanvasPointer(clientX, clientY) {
  ...
  if (activeTool === "plant") {
    ...
    if (emptyPos) {
      ...
      grid[ex][ey] = h;
      entities.push(h);
      logLine(`🟠 Hand of the Steward: Introduced grazer near (${ex}, ${ey}).`, "event");
    }
  ...
  } else if (activeTool === "detritivore") {
    const emptyPos = findNearestEmptyCell(gridX, gridY);
    if (emptyPos) {
      const [ex, ey] = emptyPos;
      const d = createDetritivore(ex, ey);
      grid[ex][ey] = d;
      entities.push(d);
      logLine(`🟣 Hand of the Steward: Placed benthic crab near (${ex}, ${ey}).`, "event");
    }
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      ...
      const cardinalTargets = [
        [(x + 1) % WIDTH, y, axonWeights[x][y].right],
        [x, (y + 1) % HEIGHT, axonWeights[x][y].down]
      ];

      for (const [nx, ny, weight] of cardinalTargets) {
        ...
        if (neighbor && neighbor.type === "plant") {
          ...
          if (activePulse > 0.05) {
            ...
            ctx.lineWidth = 1.6 + (weight - 1.0) * 1.5 + activePulse * 1.5;
            ctx.shadowColor = "#00f0ff";
            ctx.shadowBlur = 6 * activePulse;
          } else {
            const alpha = 0.12 + (weight - 1.0) * 0.25;
            ctx.strokeStyle = `rgba(16, 185, 129, ${alpha})`;
            ctx.lineWidth = 0.75 + (weight - 1.0) * 0.8;
            ctx.shadowBlur = 0;
          }
...
function renderPheromones() {
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      const phero = alarmPheromones[x][y];
      if (phero > 0.1) {
        ctx.fillStyle = `rgba(244, 63, 94, ${Math.min(0.4, phero * 0.08)})`;
...
function renderWorld() {
  ...
  for (let x = 0; x < WIDTH; x++) {
    for (let y = 0; y < HEIGHT; y++) {
      ...
      if (!e) {
        if (detritus > 0.8) {
          const detritusTone = Math.min(24, 10 + detritus * 2);
      ...
      if (e.type === "plant") {
        ...
        const isArmored = e.genome.morphotype === "armored";
        const isSprinter = e.genome.morphotype === "sprinter";
        const hue = isArmored ? 40 : (isSprinter ? 18 : 28);
        ...
        const isAmbush = e.genome.morphotype === "ambush";
        const energyTone = Math.min(65, 42 + e.energy * 1.2);
        ctx.fillStyle = isAmbush ? `hsl(320, 95%, ${energyTone}%)` : `hsl(345, 95%, ${energyTone}%)`;
        ctx.fillRect(px + 1, py + 1, CELL_SIZE - 2, CELL_SIZE - 2);
      } else if (e.type === "detritivore") {
  ...
  for (let i = activeRipples.length - 1; i >= 0; i--) {
    ...
    else if (r.tool === "detritivore") strokeColor = "rgba(192, 132, 252, ";
...
function renderSparkline() {
  ...
  for (const pt of timeSeriesHistory) {
    ...
    if (pt.d * 3 > maxVal) maxVal = pt.d * 3;
  }
  maxVal *= 1.1;
